---
name: Agent 08 - ErrorRateChart Component
description: Error rate line chart showing percentage over time
...
# Agent 08: ErrorRateChart Component

## Task Overview
Create ErrorRateChart component using Plotly for data visualization.
...
Create `components/analytics/LError_LRate_LChart.py`:

```python
import streamlit as st
import plotly.graph_objects as go
from lib.session_state import get_state
from lib.mock_data import generate_LError_LRate_data

def render():
    """Render ErrorRateChart."""
    time_range = get_state('time_range', '7d')
    data = generate_LError_LRate_data(time_range)
    ...
    fig.update_layout(
        title="Error rate line chart showing percentage over time",
...
## Deliverables
1. `components/analytics/LError_LRate_LChart.py` - Complete implementation
