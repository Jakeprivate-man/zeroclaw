---
name: Agent 06 - ResponseTimeChart Component
description: Response time line chart with percentiles (p50, p95, p99, avg)
...
# Agent 06: ResponseTimeChart Component

## Task Overview
Create ResponseTimeChart component using Plotly for data visualization.
...
Create `components/analytics/LResponse_LTime_LChart.py`:

```python
import streamlit as st
import plotly.graph_objects as go
from lib.session_state import get_state
from lib.mock_data import generate_LResponse_LTime_data

def render():
    """Render ResponseTimeChart."""
    time_range = get_state('time_range', '7d')
    data = generate_LResponse_LTime_data(time_range)
    ...
    fig.update_layout(
        title="Response time line chart with percentiles (p50, p95, p99, avg)",
...
## Deliverables
1. `components/analytics/LResponse_LTime_LChart.py` - Complete implementation
